...
int ArtificialNeuralNetwork::computeResult(double* data)
	{
		...
		for (int i = 1; i < lAmount; i++)
		{
			...
			for (int j = 0; j < j_limit; j++)
			{
				...
				{
					(*Neurons[_layer + j]).value += (*Neurons[prev_layer + k]).value * (*Neurons[prev_layer + k]).weights[j];
				}
...
void ArtificialNeuralNetwork::learn()
	{
		...
		for (int i = lAmount - (obias == true ? 0 : 1); i > 0; i--)
		{
			...
			for (int j = 0; j < j_limit; j++)
			{
				...
				if (i < lAmount) for (int k = 0; k < k_limit; k++)
				{
					...
					{
						m = (*Neurons[prev_layer + j]).m[k] = beta1*(*Neurons[prev_layer + j]).m[k] + (1 - beta1)*gradient*value;
						v = (*Neurons[prev_layer + j]).v[k] = beta2*(*Neurons[prev_layer + j]).v[k] + (1 - beta2)*pow(gradient * value, 2.0);
						(*Neurons[prev_layer + j]).weights[k] -= ratio * (m / (1 - pow(beta1, t)) / (sqrt(v / (1 - pow(beta2, t))) + eps));
						t++;
					}
