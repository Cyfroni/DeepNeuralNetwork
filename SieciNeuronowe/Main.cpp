...
double test_patients(ArtificialNeuralNetwork*, vector<double>*, const int, const int, const int, int*, const int = 0, bool = false, bool = true, bool = false);
...
void test_excel(string path)
{
	...
	while (1)
	{
		...
		while (1)
		{
			cout << "NEW SCHUFF ACTIV INIT UPDATE LOSS / EDIT(-) / BACKUP(b) / SAVE(s/[0-9]s) / LOAD(l/[0-9]l) / FULL(f/lf) / RESET(r) / DIVIDE((r,l, )/)\n";
			getline(cin, info);
			if (info.length() > 0)
			{
				...
				else if (info[info.length() - 1] == '-')
				{
					cout << "test_lines iterations verify_lines ratio lambda decay evenly\n"
						<< test_lines << " " << iterations << " " << verify_lines << " " << getRatio() << " " << getLambda() << " " << decay << " " << evenly << "\n";
					getline(cin, info);
...
double test_patients(ArtificialNeuralNetwork* A, vector<double>* vec, const int test_begin, const int test_lines, const int iterations, int* shuffled_lines, const int decay, bool learning, bool writing, bool evenly)
{
	long double loss;
	int correct;
